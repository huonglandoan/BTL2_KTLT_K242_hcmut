...
public:
    ...
    Unit *clone() const override{   return new Infantry(getQuantity(), getWeight(), getCurrentPosition(), getInfantryType());  }
    ...
    void setAttackScore(int score) {    modifiedScore = score; }
    void resetAttackScore() {  modifiedScore = -1;  }
};
...
class Army {
    ...
    public:
        ...
        virtual void checkRange(int& num, int MAX) const {  num = std::max(0, std::min(num, MAX));   }
    
        virtual bool isARVNArmy() const { return false; }
        virtual bool isLiberationArmy() const { return false; }
    };
